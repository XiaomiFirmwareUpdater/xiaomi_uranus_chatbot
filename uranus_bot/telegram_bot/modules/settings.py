...
from uranus_bot.telegram_bot.messages.miui_updates import subscriptions_message
...
#     """Send a message when the command /setlang is sent."""
...
@BOT.on(events.NewMessage(pattern='/setlang'))
async def setlang_keyboard(event):
    """Send a message when the command /setlang is sent"""
    locale = DATABASE.get_locale(event.chat_id)
    if not await subscription_allowed(event):
        return
